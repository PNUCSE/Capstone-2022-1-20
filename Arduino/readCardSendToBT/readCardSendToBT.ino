...
void loop(void) {
  ...
  if (success) {
    ...
    // Send UID key through BT
    String BTchar = tagToString(uid, uidLength);
    Serial.println(BTchar);
    BTSerial.println(BTchar);
...
String tagToString(byte *uid, uint32_t uidLength) {
  String tagID = "";
  for (uint32_t i = 0; i < uidLength; i++) {
    if (i < (uidLength - 1)) {
      tagID += String(uid[i]) + ",";
    } else {
      tagID += String(uid[i]);
